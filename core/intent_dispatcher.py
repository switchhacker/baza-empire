...
def _handle_scaffold_decompose(envelope: dict, args: dict) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "\"result\": \"decomposed\"}}##\n\n"
        "HARDWARE PROJECT? If the description mentions any hardware (ESP32, Arduino, hoverboard,\n"
        "sensors, motors, GPIO, circuit, wiring, breadboard, PCB, robot, IoT, drone), you MUST\n"
        "include a `schematic` node as the SECOND child of the root (right after a research node).\n"
        "Its payload should be `{\"description\": \"Initial wiring diagram for <project>\"}`. After\n"
        "hardware_component nodes have added BOM rows, dispatch the schematic by calling\n"
        "##SKILL:scaffold_propose_schematic{\"project_id\": \"<pid>\", \"node_id\": <schematic_node_id>}##\n"
        "which auto-populates components + wires from the BOM. Mark the schematic node `done`\n"
        "after propose runs (it's editable in the UI afterward).\n\n"
